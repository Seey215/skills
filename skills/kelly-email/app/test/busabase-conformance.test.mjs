...
test("busabase conformance (live local server)", { skip: !LIVE_URL && "set BUSABASE_LIVE_TEST_URL to run" }, async (t) => {
  ...
  const server = await new Promise((resolve) => {
    const instance = serve({ fetch: app.fetch, hostname: "127.0.0.1", port: 0 }, (info) => resolve({ instance, port: info.port }));
  });

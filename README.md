...
New Busabase operating-app skills use the repository's `kelly-app-skill-creator` contract: Research, Plan, Action, and Retrospective product workflows delegate technical implementation to `$busabase` and `$busabase-app-creator`. Existing local App-in-Skill workflows retain their audited implementation baseline:
...
| `kelly-app-skill-creator` | Designs Busabase-only human-and-Agent operating apps around Research, Plan, Action, and Retrospective, delegating connection and implementation to `$busabase` and `$busabase-app-creator`. The previous names `kelly-app-creator` and `app-in-skill-creator` remain compatibility aliases. | Use it when building a Busabase research desk, review queue, planner, action console, operating dashboard, control panel, or collaboration workspace. | [View ↗](https://mr-kelly.github.io/skills/s/kelly-app-skill-creator.html) |
...
| `kelly-invest-stock` | Designs a Busabase-only, read-only China stock portfolio and research desk using exact-pinned, no-key JavaScript market data from `stock-sdk`. It keeps user-entered holdings separate from public prices and never trades. | Use it when monitoring mainland A-share holdings, related indices or stock ETFs, refreshing quotes and K-lines, or reviewing P/L, concentration, drawdown, unusual moves, research hypotheses, and data freshness. | [View ↗](https://mr-kelly.github.io/skills/s/kelly-invest-stock.html) |

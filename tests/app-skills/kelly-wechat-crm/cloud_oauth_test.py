...
def run_cloud_oauth(config: dict) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-wechat-crm-cloud-home-") as app_home:
        env = {
            "HOME": app_home,
            "PORT": str(port),
            "BUSABASE_BASE_URL": "",
        ...
        with managed_process(["node", "server.js"], APP_ROOT, env, f"{app_url}/health"):
            with sync_playwright() as playwright:
                ...
                try:
                    ...
                    space_form = page.locator("[data-space-form]")
                    if space_form.count():
                        if not config["space_id"]:
                            raise AssertionError(
                                "KELLY_APP_CLOUD_TEST_SPACE_ID is required when the test account has multiple Spaces"
                            )
                        space_form.locator('select[name="space_id"]').select_option(config["space_id"])
                        space_form.get_by_role("button", name="Use this Space").click()
                        page.wait_for_load_state("networkidle")
